...
__global__ void init(T* x, const T value, const int N) {
    const int n = threadIdx.x + blockIdx.x * blockDim.x;  // 将2D/3D的线程布局映射为一维数组索引n（线程在bolck中的编号 + block在grid中的编号 * block中线程的数量）
...
__global__ void calDistKernel(
    ...
    int n = threadIdx.x + numFeatures * blockIdx.x;  // 指向样本 blockIdx.x 的 threadIdx.x 维度
    int m = threadIdx.x + numFeatures * clusterNo;  // 指向聚类中心 clusterNo 的 threadIdx.x 维度
    extern __shared__ float s_c[];  // 声明一个共享内存数组 s_c，长度在 kernel 启动时动态指定。
    s_c[threadIdx.x] = 0.0;
    // 计算每个样本到当前类的距离
    ...
    __syncthreads();
    // 开始进行规约
    for (int offset = blockDim.x >> 1; offset >= 32; offset >>= 1) {  // 标准归约操作，从两两合并开始，逐层减少线程
